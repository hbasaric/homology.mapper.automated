...
public class LocalHomologyConverter {
	...
	public static void main(String[] args) {
		// convert pathways for all WP ids
		if(args.length ==1) {
			try {
		...
		if(args.length ==2) {
			try {
				...
				Integer i = 0;
				for (Organism org : orgList) {
					converter.convertPathways(conf, i, args[1]);
					i = i+1;
				}
	...
	// Convert Pathways for a specific WP id
	private void convertPathways(Configurations conf, Integer i, String wpID) throws IOException {
		...
		Map<String, Pathway> pathways = PathwayReader.readPathways(conf.getSourceOrganism(), conf.getInputDir(), wpID);
